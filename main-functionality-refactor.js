...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const popupNextStepBtn = document.querySelector(
    ".bundle_next-step [next-step-btn]"
  );
  console.log("we've got the button", popupNextStepBtn);
  ...
  const bundleProductImg = document.querySelector(".bundle_steps_product-img")
    .src;
  ...
  const text = document.querySelector("[guide-text]");
  const textContainer = document.querySelector("[guide-text-container]");
  ...
  if (startButton) {
    startButton.addEventListener("click", async function () {
      const animationSequence = [
        ...
        isMobile() ? [bunny, { opacity: 0 }, { at: "<", duration: 0.84 }] : [],
      ...
      if (clickCount === 1) {
        ...
        introMapSteps.forEach((step, index) => {
          if (state.prizeSteps.includes(index)) {
            ...
            removePrizeOverlay(step);
          }
        });
      } else if (clickCount === 2) {
        closeScroll();
        introMapSteps.forEach((step) => {
          addPrizeOverlay(step);
        });
      }
    });
  }
  ...
  function closeScroll() {
    ...
    const closeScroll = [
      ...
      [(products, { opacity: 1 }, { duration: 0.3, at: "<", easing: fastEase })]
    ];
  ...
  const textConditions = [
    {
      condition: (tracker) => !tracker.firstProduct,
      text: "Add first item to bundle"
    },
    {
      condition: (tracker) => tracker.firstProduct && !tracker.secondProduct,
      text: "Awesome! Add 1 more item to get your Mystery Prize ⭐️"
    },
    {
      condition: (tracker) => tracker.secondProduct && !tracker.firstPrize,
      text: "What's in the treasure chest? 🏴‍☠️"
    },
    {
      condition: (tracker) => tracker.firstPrize && !tracker.thirdProduct,
      text: "Skyward Prize is 1 item away 😶‍🌫️"
    },
    {
      condition: (tracker) => tracker.thirdProduct && !tracker.secondPrize,
      text: "Claim your prize"
    },
    {
      condition: (tracker) => tracker.secondPrize && !tracker.fourthProduct,
      text: "Exciting, 1 more item till the Ultimate Enchanted Prize 🍄"
    },
    {
      condition: (tracker) => tracker.fourthProduct && !tracker.thirdPrize,
      text: "Claim your prizes ⛺️"
    }
  ];
  ...
  function addProductToBundle(productBlocks) {
    ...
    state.bundle = processProductBlocks(
      productBlocks,
      state.currentStep,
      state.editingStep
    );
    ...
    const step =
      state.editingStep === null ? state.currentStep : state.editingStep;
    ...
    if (
      !isPrizeStep(state.currentStep + 1) &&
      state.currentStep !== state.prizeSteps[2]
    ) {
      moveToNextStepClick();
    }
  }
  ...
  function removeProduct(stepToRemove, bundleProduct) {
    ...
    state.bundle = state.bundle.filter(
      (product) => product.step < stepToRemove
    );
    ...
    if (state.currentStep >= state.prizeSteps[1]) {
      console.log(
        "the current step is the second prize step and a product has been added"
      );
      ...
      console.log(
        "Total compare price with discount applied",
        totalComparePriceWithDiscount
      );

      bundleTotalEl.textContent = discountedTotal.toFixed(2);
      bundleComparePriceEl.textContent = totalComparePriceWithDiscount.toFixed(
        2
      );
    }
  ...
  function moveToNextStepClick() {
    console.log(
      "Next Step Button Clicked: Initial editingStep =",
      state.editingStep,
      "currentStep =",
      state.currentStep
    );
    ...
    if (isProductAddedForStep(state.currentStep)) {
      state.currentStep++;
      console.log(
        "Next Step Button Clicked: After Increment currentStep =",
        state.currentStep
      );
    ...
    console.log(
      "Next Step Button Clicked: After Reset editingStep =",
      state.editingStep
    );
  }
  ...
  function dontMoveToNextStep() {
    closeNextStepPopup();
    if (
      isProductAddedForStep(state.currentStep) &&
      state.currentStep != state.prizeSteps[2]
    ) {
  ...
  function updateUIAfterProductAdded(productBlocks) {
    ...
    clearTitleOverlay(mapStepsItems);
    ...
    popupNextStepBtn.style.display = "flex";
    ...
    if (
      ...
      console.log(
        "found the remove btn",
        removeBundleProductBtn[state.currentStep]
      );
      ...
      updatePopup(popupDataCurrent);
      if (popupNextStepBtn) {
        console.log("button shouldn't show?");
        popupNextStepBtn.style.display = "none";
      }
    }
    ...
    if (
      ...
      setTimeout(() => openNextStepPopup(), isMobile() ? 1100 : 460);
    }
  }
  ...
  function updateUIforRemoveProduct(currentStep) {
    ...
    bundleStepsItems.forEach((item, index) => {
      ...
      if (
        (index > currentStep && index !== state.prizeSteps[1]) ||
        (index === currentStep && index !== state.prizeSteps[1])
      ) {
  ...
  function updateUIAfterMoveToNextStep() {
    ...
    if (state.prizeSteps.includes(state.currentStep)) {
      removePrizeOverlay(bundleStepsItems[state.currentStep]);
      removePrizeOverlay(mapStepsItems[state.currentStep]);
    }
  }
  ...
  function getBundleProductsInsideBundleItem() {
    ...
    const targetStep =
      state.editingStep !== null ? state.editingStep : state.currentStep;
  ...
  function isProductAddedForStep(stepIndex) {
    ...
    const stepElement = document.querySelector(
      `[bundle-step="${stepIndex + 1}"]`
    );

    if (stepElement) {
      const bundleStepsProductElement = stepElement.querySelector(
  ...
  function getComparePriceFromBlock(productBlock) {
    ...
    if (comparePriceEl) {
      ...
      const comparePriceNumber = parseFloat(
        comparePriceText.replace(/[^\d.]/g, "")
      );
  ...
  function processProductBlocks(productBlocks, currentStep, editingStep) {
    ...
    productBlocks.forEach((productBlock) => {
      ...
      if (idNumber !== null) {
        updatedBundle = upsertProductInBundle(
          updatedBundle,
          step,
          idNumber,
          price,
  ...
  function upsertProductInBundle(
    bundle,
    step,
    idNumber,
    price,
    ...
    if (stepIndex !== -1) {
      ...
      if (isLastStep && !bundle[stepIndex].productInfo.includes(idNumber)) {
        bundle[stepIndex].productInfo.push({
          productId: idNumber,
          productPrice: price,
  ...
  function typeWriterAnimation(milestoneText, index) {
    const words = milestoneText.split(" ");
    if (index < words.length) {
      // Create a span for the word with the slide-up animation
      const wordSpan = document.createElement("span");
      wordSpan.textContent = words[index];
      wordSpan.classList.add("word-slide-up");
      ...
      setTimeout(function () {
        typeWriterAnimation(milestoneText, index + 1);
  ...
  function textForCurrentMilestone(tracker) {
    if (state.editingStep !== null) {
      return "Switch your product for this step";
    }

    for (const condition of textConditions) {
      if (condition.condition(tracker)) {
        return condition.text;
      }
    }
    return "All steps completed"; // Fallback text
  }

  function updateMilestoneText() {
    // Flatten the milestone object for easier condition checking
    const flattenedTracker = stepsTracker.milestones.reduce(
      (acc, curr) => ({ ...acc, ...curr }),
      {}
    );
    ...
    // Get the text for the current milestone
    const milestoneText = textForCurrentMilestone(flattenedTracker);

    // Call the typewriter animation with the new text
    typeWriterAnimation(milestoneText, 0);
  }
  ...
  function clearTitleOverlay(bundleStepsItems) {
    const targetBundleItem =
      state.editingStep !== null
        ? bundleStepsItems[state.editingStep - 1]
        : bundleStepsItems[state.currentStep];
  ...
  function removePrizeOverlay(bundleStepItem) {
    const prizeOverlay = bundleStepItem.querySelector("[prize-overlay]");
    console.log("prize overlay", prizeOverlay);
    prizeOverlay.classList.add("is--closed");
  }

  function addPrizeOverlay(bundleStepItem) {
    const prizeOverlay = bundleStepItem.querySelector("[prize-overlay]");
    prizeOverlay.classList.remove("is--closed");
  }

  function activateStepImage(stepItems) {
    const stepImageEl = stepItems.querySelector("[step-image]");
  ...
  function updateMobileBundleStepInfo(stepElement) {
    const mobileIndicator = document.querySelector("[bundle-mobile-indicator]");
    ...
    const iconImgData = stepElement.dataset.stepIconImg;
    const locationData = stepElement.dataset.stepIconImg;

    if (iconImgEl) {
      iconImgEl.src = iconImgData;
      iconImgEl.srcset = iconImgData;
    }
